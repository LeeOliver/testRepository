...
@property (unsafe_unretained, nonatomic) NSString *runLoopMode;
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        [NSInvocation performSelectorOnMainThread:@selector(invoke) withObject:nil waitUntilDone:[NSThread isMainThread]];
    }
}
...
- (void)runPACScript:(NSString *)script
{
	if (script) {
		...
		CFErrorRef err = NULL;
		NSArray *proxies = NSMakeCollectable([(NSArray *)CFNetworkCopyProxiesForAutoConfigurationScript((__bridge CFStringRef)script,(CFURLRef)[self url], &err) autorelease]);
